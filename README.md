...
The **purpose** of this project is to provide the necessary tools to manage paths and references during the conversion of a segmentation in a research file formats (`.nrrd`, `.nii`, etc.) to a `DICOM SEG` file.  The main goal is accomplished by using the [pydicom-seg](https://pypi.org/project/pydicom-seg/) library to create the `DICOM SEG` files.
