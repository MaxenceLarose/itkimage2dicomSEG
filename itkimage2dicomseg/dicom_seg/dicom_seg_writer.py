"""
    ...
    @Description:       This file contains the DicomSEGWriter which is used to create DICOM SEG files from segmentations
...
class DicomSEGWriter:
    ...
    def __init__(
        ...
        self.series_ids = self.__get_series_ids()
        self.series_data_dict = self.__get_series_data_list()
    ...
    def paths_to_segmentations(self) -> List[str]:
        ...
        for path in os.listdir(self.path_to_segmentations_folder):
            path_to_segmentations.append(os.path.join(self.path_to_segmentations_folder, path))
        return path_to_segmentations

    def __get_series_ids(self) -> List[str]:
        """
    ...
    def __get_series_data_list(self) -> List[SeriesData]:
        """
    ...
    def _template(self) -> pydicom.Dataset:
        ...
        template : pydicom.Dataset
            Metadata template.
        """
        return pydicom_seg.template.from_dcmqi_metainfo(self.path_to_metadata_json)
    ...
    def __get_dicom_series_paths_for_given_segmentation(self, path_to_segmentation: str) -> List[str]:
        """
        Print the patient's UID and its series descriptions (and index).
        ...
        self._display_series_list(series_data_list=self.series_data_dict)
        chosen_series = None
        while True:
            try:
                ...
                chosen_series = self.series_data_dict[int(series_idx)]
    ...
    @staticmethod
    def __get_sitk_label_map_for_given_segmentation(path_to_segmentation: str) -> sitk.Image:
        """
    ...
    @staticmethod
    def _is_segmentation_association_process_complete():
        """
    ...
    def write(
        ...
        for path_to_seg in self.paths_to_segmentations:
            process_complete = False
            i = 0
            while not process_complete:
                source_images = self.__get_dicom_series_paths_for_given_segmentation(path_to_seg)
                segmentation = self.__get_sitk_label_map_for_given_segmentation(path_to_seg)
                ...
                new_path = f"{os.path.join(self.path_to_segmentations_folder, pathlib.Path(path_to_seg).stem)}_" \
                           f"{i}.SEG.dcm"
                ...
                if enable_multi_images_association:
                    process_complete = self._is_segmentation_association_process_complete()
                    i += 1
                else:
